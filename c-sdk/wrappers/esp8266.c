...
#define ESP8266_MAX_PAYLOAD_SIZE (CONFIG_MQTT_MESSAGE_MAXLEN + CONFIG_MQTT_TOPIC_MAXLEN + 20)

#ifndef PLATFORM_HAS_DYNMEM
static uint8_t payload[ESP8266_MAX_PAYLOAD_SIZE] = {0};
#endif
void *coap_udpmutex;
 ...
 * +WEVENT:STATION_DOWN
 */
static void esp8266wifi_event_handler(void *arg, char *buf, int buflen)
{
    char eventhead[4] = {0};
    char eventotal[16] = {0};
...
static void esp8266_uart_echo_off()
{
...
int HAL_AT_CONN_Init(void)
{
    ...
    at_register_callback(WIFIEVENT_OOB_PREFIX, NULL, NULL, 0, esp8266wifi_event_handler, NULL);
...
static char mac_str_8266[HAL_MAC_LEN];

char *HAL_Wifi_Get_Mac(char mac_str[HAL_MAC_LEN])
{

	if(strlen(mac_str_8266)){
		strncpy(mac_str,mac_str_8266,strlen(mac_str_8266));
		return 0;
	}
