...
static void wm_init(void )
{
    ...
    {
        wm_second_timer = xTimerCreate("wm_second_timer", 
                                       pdMS_TO_TICKS(WM_CBTIMER_PERIOD_MS), pdTRUE, NULL, (TimerCallbackFunction_t)wm_s_timer_cb);
...
//    app_wait_wifi_connect();
	...
	report_started = true;
	wm_report_all_pro_to_cloud();	//Pause here
    while (1) {
        
        IOT_Linkkit_Yield(EXAMPLE_YIELD_TIMEOUT_MS);	//TODO: Check how it works for shifting thread between CPU and sdk
